# baseline_core.py
# Core backtest engine used by scripts/baseline_pair_runner.py
# EURUSD/GBPUSD M1 backtest:
...
def _dump_config_fingerprint():
    print("\nCONFIG FINGERPRINT:")
    print(f" DATA_CSV_PATH = {DATA_CSV_PATH}")
    print(f" ACCOUNT_START = {ACCOUNT_START}")
    print(f" RISK_PER_TRADE = {RISK_PER_TRADE}")
    print(f" ATR_MULT_SL = {ATR_MULT_SL}")
    print(f" ATR_MULT_TP = {ATR_MULT_TP}")
    print(f" MAX_BARS_HOLD = {MAX_BARS_HOLD}")
    print(f" RSI_BUY_MAX = {RSI_BUY_MAX} | RSI_SELL_MIN = {RSI_SELL_MIN}")
    print(f" RSI_WINDOW_AHEAD = {RSI_WINDOW_AHEAD}")
    print(f" ATR_BAND_LOOKBACK = {ATR_BAND_LOOKBACK} | ATR_P_LOW = {ATR_P_LOW} | ATR_P_HIGH = {ATR_P_HIGH} | ATR_MIN_WARMUP = {ATR_MIN_WARMUP}")
    print(f" EMA_TREND_RECENT = {EMA_TREND_RECENT}")
    print(f" USE_BREAKEVEN = {USE_BREAKEVEN} | BE_MULT = {BE_MULT} | BE_MIN_BARS_HELD = {BE_MIN_BARS_HELD}")
    print(f" USE_SESSION_FILTER = {USE_SESSION_FILTER} | HOURS {SESSION_START_HHMM}-{SESSION_END_HHMM} | WEEKDAYS_ONLY={TRADE_WEEKDAYS_ONLY}")
    print(f" EXCLUDE_UTC_HOURS = {sorted(list(EXCLUDE_UTC_HOURS))} | EXCLUDE_WEEKDAYS = {sorted(list(EXCLUDE_WEEKDAYS))}")
    print(f" DISABLE_LONG_COMBO = {DISABLE_LONG_COMBO} | RSI_LONG_BAD_CUTOFF = {RSI_LONG_BAD_CUTOFF}")
    print(f" USE_APRIL_ATR_TIGHTENING = {USE_APRIL_ATR_TIGHTENING} | APRIL_ATR_P_LOW = {APRIL_ATR_P_LOW}")
    print(f" MAX_DAILY_DD = {MAX_DAILY_DD} | MAX_TOTAL_DD = {MAX_TOTAL_DD}")
    print()
...
# ========= Data / Strategy Config (defaults; runner may override path/pair) =========
DATA_CSV_PATH   = "data/clean/EURUSD/2023/EURUSD_1y_2023_clean.csv"
...
EXCLUDE_UTC_HOURS  = {11, 12}
EXCLUDE_WEEKDAYS   = set()

# April/robustness combo guard
DISABLE_LONG_COMBO  = True
...
# (Optional) dynamic wider SL — not wired into execution path here
DYN_SL_ENABLE          = False
...
# ========= Pretty monthlies (re-usable) =========
def print_monthlies(monthly, account_start, year_hint=None):
    if not monthly:
        print("⚠️ No monthly results were collected.")
        return
    first_key = sorted(monthly.keys())[0]
    year = year_hint if year_hint else int(first_key.split("-")[0])
    ...
    for mkey in ordered_months:
        m = monthly.get(mkey, {"pl":0.0,"trades":0,"wins":0,"losses":0,"profits":[],"losses_list":[]})
        trades, wins, losses, pl = m["trades"], m["wins"], m["losses"], m["pl"]
        win_rate = (wins / trades * 100.0) if trades else 0.0
        profit_sum = sum(m["profits"]); loss_sum = sum(m["losses_list"])
        pf = (profit_sum / loss_sum) if loss_sum > 1e-12 else (float('inf') if profit_sum > 0 else 0.0)
        ret_pct = (pl / account_start) * 100.0
        print(f"{mkey:7} | {trades:3d} | {wins:3d} | {losses:3d} | {win_rate:5.1f} | ${pl:10.2f} | {ret_pct:6.2f}% | {pf:4.2f}")

# ========= Backtest =========
def main(return_results=False, override_data_csv_path=None, override_pair=None):
    # Resolve data path and pair
    data_csv_path = override_data_csv_path or DATA_CSV_PATH
    pair = override_pair or "UNKNOWN"

    # Try to infer year from path (…/YYYY/…); fall back later
    inferred_year = None
    parts = os.path.normpath(data_csv_path).split(os.sep)
    for p in parts:
    ...
    # Only print headers when running the core directly.
    if not return_results:
        print(f"PAIR={pair} YEAR={inferred_year if inferred_year else 'n/a'}")
        print(f"Using DATA_CSV_PATH: {os.path.abspath(data_csv_path)}")
        ...
        _dump_config_fingerprint()

    data = load_ohlcv_clean(data_csv_path)
    total_bars = len(data)
    if not return_results:
        print(f"Loaded {total_bars} candles from {os.path.abspath(data_csv_path)}")

    # Prepare runs/<PAIR>/<YEAR> and trades csv path
    runs_dir = os.path.join("runs", pair, str(inferred_year) if inferred_year else "unknown")
    os.makedirs(runs_dir, exist_ok=True)
    trades_csv_path = os.path.join(runs_dir, f"trades_log_{run_id}.csv")
    ...
    while i < total_bars:
        ...
        # Progress ping
            if not return_results and FAST_MODE and (i % PROGRESS_EVERY == 0) and i > 0:
        ...
        while j < total_bars:
            ...
            if direction == "buy":
                ...
                if hit_stop and hit_tp:
                    ...
                    exit_price = target; exit_reason = "target"; break
            # Optional breakeven nudge after some progress (frozen baseline style)
            if USE_BREAKEVEN:
                if direction == "buy":
                    peak = max(peak, hj)
    ...
    results = {
        ...
        "funnel": dict(funnel),
        "examples": examples,
        "trades_csv": trades_csv_path if LOG_TRADES else None,
        "summary": {
            "trades": n_trades,
            "wins": n_wins,
            "losses": n_losses,
            "wl_ratio": (n_wins / n_losses) if n_losses > 0 else (float(n_wins) if n_wins else 0.0),
    ...
    # ======== Signal Funnel Summary / Printing ========
    if not return_results:
        ...
        # ======== MONTHLY PERFORMANCE ========
        # Infer year for printout if we couldn't from path
        if monthly:
            first_key = sorted(monthly.keys())[0]
            print_year = inferred_year if inferred_year else int(first_key.split("-")[0])
        else:
            print_year = inferred_year if inferred_year else None

        print_monthlies(monthly, ACCOUNT_START, year_hint=print_year)

    # ======== Backtest Summary numbers (used in prints and returned dict) ========
    cum_ret = (portfolio - ACCOUNT_START) / ACCOUNT_START
    wl = (n_wins / n_losses) if n_losses > 0 else (float(n_wins) if n_wins else 0.0)
    pf = (gross_profit / gross_loss) if gross_loss > 0 else float('inf')
    expectancy = ((gross_profit - gross_loss) / max(n_trades, 1))

    # If we're running as a script, print the summary
    if not return_results:
    ...
    # ======== Assemble results for caller (runner/UI) ========
    results = {
        "monthly_results": monthly,            # dict keyed "YYYY-MM" -> stats
        "account_start": ACCOUNT_START,
        "funnel": dict(funnel),                # optional; used by --verbose
        "examples": examples,                  # optional; used by --verbose
        "trades_csv": trades_csv_path if LOG_TRADES else None,
        "summary": {
            "trades": n_trades,
            "wins": n_wins,
