...
ACCOUNT_START   = 200_000.00
RISK_PER_TRADE  = 0.005        # 0.5% per trade
MACD_EPS        = 1e-6
ATR_MULT_SL     = 1.5
ATR_MULT_TP     = 2.1
MAX_BARS_HOLD   = 240          # ~5 hours on M1
...
RSI_WINDOW_AHEAD = 6           # allow RSI to confirm within +6 bars from cross (no look-ahead)

# ======== ATR gate via rolling percentile ========
ATR_BAND_LOOKBACK = 1000       # ~1.5 days of M1
ATR_P_LOW         = 0.26       # keep trades when ATR in middle regime
ATR_P_HIGH        = 0.90
ATR_MIN_WARMUP    = 200        # permissive until warmup satisfied

# ======== EMA “recent” trend window ========
EMA_TREND_RECENT  = 6          # trend is OK if seen on any of last N bars

# ======== Breakeven (optional) ========
USE_BREAKEVEN   = True
BE_MULT         = 0.85         # move stop to entry after +0.9 * ATR at entry in unrealized
...
# ========= Risk Guards =========
MAX_DAILY_DD    = 0.038        # 3.8% from start-of-day equity
MAX_TOTAL_DD    = 0.07         # 7% from peak equity

# ======== April fixes / robustness guards ========
EXCLUDE_UTC_HOURS      = {11, 12}   # block entries during 11:00–12:59 UTC
EXCLUDE_WEEKDAYS       = set()      # e.g., {0} to exclude Mondays; keep empty by default
DISABLE_LONG_COMBO     = True       # skip LONG when ema50>=ema200 AND RSI<=30
RSI_LONG_BAD_CUTOFF    = 30.0

# Dynamic stop sizing in elevated vol (mid–high ATR regime)
DYN_SL_ENABLE          = True
ATR_PCNT_WIDE_SL_FROM  = 0.70       # widen SL if ATR >= 70th percentile
ATR_MULT_SL_BASE       = 1.5        # your base SL
ATR_MULT_SL_WIDE       = 2.0        # wider SL for elevated ATR

# Optional: minimum bars before we allow breakeven to arm
BE_MIN_BARS_HELD       = 15
